---
description: "Introduction scaffold for explanation-type articles"
---

# Pattern: Explanation introduction

> Scaffold for explanation articles (understanding-oriented). Copy, adapt, and delete the placeholder comments.

```markdown
# Understanding Concept

> Why this concept matters — the value proposition

## Table of Contents
[...]

## Introduction

[Concept] is fundamental to [larger topic]. Understanding it helps you [benefit].

This article explores:
- **Core principle 1** — [Brief description]
- **Core principle 2** — [Brief description]
- **Application** — [How to use this understanding]

**Prerequisites:** [Links to foundational knowledge]
```

---

**Source:** Diátaxis — explanations are understanding-oriented. See `article-writing.instructions.md` for voice and formatting rules.

<!--
---
template_metadata:
  version: "1.0.0"
  last_updated: "2026-03-20"
  created: "2026-03-20"
  consumers: []
  changelog: "pattern-explanation-intro.template.changelog.md"
---
-->
