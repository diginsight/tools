---
name: pe-meta-template-review
description: "Review a PE-for-PE template across selected dimensions with schema and placeholder checks"
agent: agent
...
scope:
  covers: ["Shared quality objective and scope intent enforcement (applicability-scoped)", "Dimension-scoped review", "Schema compliance audit", "Placeholder documentation check", "Consumer compatibility"]
  excludes: ["Domain templates"]
boundaries:
  - "MUST share the same quality objective and scope intent as /pe-meta-template-design (applicability-scoped)"
  ...
  - "MUST always check schema compliance and placeholder docs"
rationales:
...
2. Resolve the applicable-dimension SET from the `05.07-pe-meta-dimension-catalog.md` applicability matrix for the `template` type — this is the `<applicable>` denominator (the full type-applicable set, NOT the `05.08` enumerated subset); then load `05.08-pe-meta-type-checklists.md` → template section for the SUB-CHECKS of those applicable dimensions that declare rows
3. Verify all placeholders documented with type and example
4. Check schema compliance with target artifact type
...
6. If `--deps full`: run full dependency-aware review (bounded recursion, default depth 2, scope-filtered) and verify consuming prompts/agents can fill all placeholders
7. If `--deps direct`: run first-level-only dependency checks (scope-filtered) and verify direct consumers can fill placeholders
...
A direct `/pe-meta-template-review` call MUST reach the **same evidence depth** as the `/pe-meta-review` orchestrator — depth is a property of the work, not the entry path. This prompt therefore includes the first **technique module** (engine **Assess** phase):

```text
#file:.github/prompt-snippets/pe-meta-evidence-coverage.md
```

**`dim_evidence[]` (MANDATORY).** For EVERY applicable dimension — **passes included** — record one `{dim, status, evidence_ref}` object with a non-empty, anchored `evidence_ref` (`path:line` + verbatim quote). A `status: pass` with an empty `evidence_ref` does NOT count as covered. The SET of applicable dimensions is the `05.07` applicability-matrix set for the `template` type (NOT the `05.08` subset); an applicable dimension with no `05.08` rows still requires one anchored `evidence_ref`. Each dimension's `evidence_ref` set MUST discharge every sub-check declared for the `template` type in [05.08-pe-meta-type-checklists.md](../../../.copilot/context/00.00-prompt-engineering/05.08-pe-meta-type-checklists.md).
