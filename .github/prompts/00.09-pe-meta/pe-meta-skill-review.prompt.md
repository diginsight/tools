...
2. Resolve the applicable-dimension SET from the `05.07-pe-meta-dimension-catalog.md` applicability matrix for the `skill` type — this is the `<applicable>` denominator (the full type-applicable set, NOT the `05.08` enumerated subset); then load `05.08-pe-meta-type-checklists.md` → skill section for the SUB-CHECKS of those applicable dimensions that declare rows
...
A direct `/pe-meta-skill-review` call MUST reach the **same evidence depth** as the `/pe-meta-review` orchestrator — depth is a property of the work, not the entry path. This prompt therefore includes the first **technique module** (engine **Assess** phase):

```text
#file:.github/prompt-snippets/pe-meta-evidence-coverage.md
```

**`dim_evidence[]` (MANDATORY).** For EVERY applicable dimension — **passes included** — record one `{dim, status, evidence_ref}` object with a non-empty, anchored `evidence_ref` (`path:line` + verbatim quote). A `status: pass` with an empty `evidence_ref` does NOT count as covered. The SET of applicable dimensions is the `05.07` applicability-matrix set for the `skill` type (NOT the `05.08` subset); an applicable dimension with no `05.08` rows still requires one anchored `evidence_ref`. Each dimension's `evidence_ref` set MUST discharge every sub-check declared for the `skill` type in [05.08-pe-meta-type-checklists.md](../../../.copilot/context/00.00-prompt-engineering/05.08-pe-meta-type-checklists.md).
