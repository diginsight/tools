---
name: pe-meta-snippet-review
description: "Review a PE-for-PE prompt snippet across selected dimensions with reusability and naming checks"
agent: agent
...
scope:
  ...
  excludes: ["Domain snippets"]
boundaries:
  - "MUST share the same quality objective and scope intent as /pe-meta-snippet-design (applicability-scoped)"
  ...
  - "MUST always check reusability threshold and naming"
rationales:
...
- **CF-05 expected root.** Any resolved target path (positional `<file-path>`, `--scope`, or design output target) MUST resolve under `.github/prompt-snippets/`. Paths outside this root are REJECTED before Phase 0b runs; the rejection message MUST suggest the canonical replacement prompt name from the SoT § Per-prompt-class applicability matrix.
...
6. If `--deps full`: run full dependency-aware review (bounded recursion, default depth 2, scope-filtered) and verify all consumers parse snippet correctly
7. If `--deps direct`: run first-level-only dependency checks (scope-filtered) and verify direct consumers parse snippet correctly
...
A direct `/pe-meta-snippet-review` call MUST reach the **same evidence depth** as the `/pe-meta-review` orchestrator — depth is a property of the work, not the entry path. This prompt therefore includes the first **technique module** (engine **Assess** phase):
