---
name: pe-meta-instruction-review
description: "Review a PE-for-PE instruction file across selected dimensions with applyTo and minimization checks"
agent: agent
...
scope:
  covers: ["Shared quality objective and scope intent enforcement (applicability-scoped)", "Dimension-scoped review", "applyTo precision audit", "Minimization check", "Overlap detection"]
  excludes: ["Domain instructions"]
boundaries:
  - "MUST share the same quality objective and scope intent as /pe-meta-instruction-design (applicability-scoped)"
  ...
  - "MUST always check applyTo precision and minimization"
rationales:
...
2. Resolve the applicable-dimension SET from the `05.07-pe-meta-dimension-catalog.md` applicability matrix for the `instruction` type — this is the `<applicable>` denominator (the full type-applicable set, NOT the `05.08` enumerated subset); then load `05.08-pe-meta-type-checklists.md` → instruction section for the SUB-CHECKS of those applicable dimensions that declare rows
3. Audit applyTo pattern — count matched files, flag over-broad patterns
4. Check minimization — flag unnecessary examples, prose, or commentary
...
6. If `--deps full`: run full dependency-aware review (bounded recursion, default depth 2, scope-filtered) and check for overlap with other instruction files
7. If `--deps direct`: run first-level-only dependency checks (scope-filtered) and check direct overlap risks
