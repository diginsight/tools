---
title: "Adaptive validation patterns for prompt engineering"
description: "Challenge-based validation methodology — complexity assessment, use case challenge templates, role validation, workflow reliability testing, boundary actionability"
domain: "prompt-engineering"
goal: "Provide challenge-based validation methodology that adapts validation depth to artifact complexity"
scope:
  covers:
    - "Complexity assessment criteria (Simple, Moderate, Complex)"
    - "Use case challenge templates per complexity level"
    - "Role validation methodology"
    - "Workflow reliability testing patterns"
    - "Tool requirement mapping and boundary actionability validation"
  excludes:
    - "Validation caching (see 04.01-validation-caching-pattern.md)"
    - "Production readiness patterns (see 04.03-production-readiness-patterns.md)"
boundaries:
  - "MUST adapt validation depth to artifact complexity"
  - "MUST NOT apply deep validation to simple, low-risk artifacts"
rationales:
  - "Complexity-based depth prevents over-validation of simple tasks"
  - "Challenge-based approach catches design flaws earlier than structural checks"
---
...
📖 **Worked examples:** `.github/templates/00.00-prompt-engineering/reference-validation-examples.template.md`
...
Each test yields a PASS / FAIL (or WARN) verdict:

1. **Authority Test** — PASS if role + action + domain align (e.g. "API reviewer with OpenAPI 3.0 expertise" reviewing API specs); FAIL if role and action conflict (e.g. "junior developer" mandated to ship production infrastructure).
2. **Expertise Test** — PASS if the role name implies the domain knowledge the task requires; FAIL if a gap exists (e.g. "Python developer" reviewing Java with no Java mentioned).
3. **Specificity Test** — PASS if the role names ≥1 domain or specialty (e.g. "REST API validator"); FAIL if generic ("assistant", "helper", "bot").
4. **Pattern Search** — PASS if a similar, proven role exists in the workspace; WARN if no match found.
5. **Refinement** — PASS once the role has been adjusted to clear any failing test above.
...
2. **Cross-reference:** `.copilot/context/00.00-prompt-engineering/01.04-tool-composition-guide.md`
