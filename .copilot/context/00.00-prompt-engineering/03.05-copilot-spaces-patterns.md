---
title: "Copilot Spaces patterns"
description: "Decision framework for GitHub Copilot Spaces as persistent cross-project context — content types, when to use, complementary role with repo-level files"
domain: "prompt-engineering"
authoritative_sources:
  - url: "https://docs.github.com/en/copilot/using-github-copilot/using-copilot-spaces"
    description: "GitHub Copilot Spaces — persistent cross-project context, content types, sharing"
goal: "Define when and how to use Copilot Spaces as persistent cross-project context complementary to repository-level customization"
scope:
  covers:
    - "What Spaces are (persistent, shareable GitHub platform-level context)"
    - "Space content types (instructions, files/repos, links, uploads, text)"
    - "Where Spaces fit in customization stack"
    - "Decision guide for Space vs. repository-level approaches"
    - "Synchronization and persistence behavior"
  excludes:
    - "Repository-level artifact files (see pe-*-instructions.md)"
    - "Copilot Spaces API details (see GitHub documentation)"
boundaries:
  - "MUST use Spaces only for cross-project persistent context"
  - "MUST NOT duplicate repository-level patterns in Spaces"
rationales:
  - "Spaces fill the gap for cross-project knowledge without repo clutter"
  - "Clear scope separation prevents artifact proliferation"
---
...
**Referenced by**: Agent files using cross-project context, orchestrator prompts referencing external knowledge, `.github/instructions/pe-agents.instructions.md`
...
Spaces inject context **alongside** the prompt assembly pipeline documented in [01.02-prompt-assembly-architecture.md](./01.02-prompt-assembly-architecture.md). Space instructions act like an additional system prompt layer; Space sources provide grounding context. They don't change the 6-layer system prompt structure — they extend it.

---

## References

- **Internal**: [01.02-prompt-assembly-architecture.md](./01.02-prompt-assembly-architecture.md), [01.03-file-type-decision-guide.md](./01.03-file-type-decision-guide.md)
- **Source**: `03.00-tech/05.02-prompt-engineering/01-overview/01.01-appendix-copilot-spaces.md`
...
| 1.2.0 | 2026-06-01 | Updated feature status — Copilot Spaces API GA (2026-05-18); removed stale "No API" limitation | System |
