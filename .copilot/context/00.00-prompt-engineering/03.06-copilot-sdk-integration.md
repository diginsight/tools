---
title: "Copilot SDK integration patterns"
description: "Consuming PE artifacts from SDK-based applications outside VS Code — Node.js, Python, Go, .NET, YAML differences, billing"
domain: "prompt-engineering"
authoritative_sources:
  - url: "https://code.visualstudio.com/docs/agent-customization/overview"
    description: "VS Code Copilot customization — SDK integration patterns, YAML differences for programmatic use"
goal: "Provide decision framework for consuming PE artifacts from SDK-based applications outside VS Code"
scope:
  covers:
    - "GitHub Copilot SDK overview and availability (Node.js, Python, Go, .NET)"
    - "SDK architecture (CLI, Copilot API)"
    - "Feature comparison (VS Code vs. SDK apps)"
    - "YAML differences for programmatic use"
    - "Billing and licensing considerations"
  excludes:
    - "VS Code-specific customization (see pe-copilot-instructions.md)"
    - "Prompt/agent/instruction file authoring (see pe-*-instructions.md files)"
boundaries:
  - "MUST document feature differences for SDK consumers"
  - "MUST NOT assume VS Code-only capabilities for SDK users"
rationales:
  - "SDK extends PE artifacts beyond VS Code to any application"
  - "Feature documentation prevents user frustration on unsupported capabilities"
---
...
The **GitHub Copilot SDK** (generally available since June 2026; public preview April 2026) brings Copilot's agentic loop to any application. Available for Node.js, Python, Go, and .NET. The critical insight: **the SDK consumes the same files you already write**.

With **BYOK (Bring Your Own Key)** support (VS Code 1.117+, Copilot CLI April 2026), SDK apps can also route requests through user-provided API keys for OpenRouter, Ollama, Google, OpenAI, and other providers.
...
const triage = await client.createSession({ model: "gpt-5" });
...
- **Internal**: [01.02-prompt-assembly-architecture.md](./01.02-prompt-assembly-architecture.md), [01.04-tool-composition-guide.md](./01.04-tool-composition-guide.md), [03.04-mcp-server-design-patterns.md](./03.04-mcp-server-design-patterns.md)
- **Source**: `03.00-tech/05.02-prompt-engineering/04-howto/14.00-how-to-use-prompts-with-the-github-copilot-sdk.md`
...
| 1.1.0 | 2026-04-27 | Updated SDK status to public preview (April 2026), added BYOK model support | Release monitor |
| 1.4.0 | 2026-06-03 | Freshness refresh (D12-staleness): SDK status → generally available (2026-06-02) | Release monitor |
